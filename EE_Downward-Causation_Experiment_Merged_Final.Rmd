...
# Plot Interactions and simple slope analysis

# Standardize the moderator variables to facilitate analysis
pooled_data$oth_ori_z_std <- scale(pooled_data$oth_ori)
pooled_data$self_int_z_std <- scale(pooled_data$self_int)

# Fit the Model with Z-standardized Moderator Variables

multilevel_model_rirs_cli_plot <- lmer(eecontlike ~ ee_support_reception + ee_affiliation + ee_relationships + 
                                  altruism_potential + ee_gain_perception + benefit_perception + 
                                  age + compage + gender + 
                                  oth_ori_z_std * altruism_potential + 
                                  self_int_z_std * benefit_perception + 
                                  (1 + oth_ori_z_std + self_int_z_std | respondent_id), 
                                  data = pooled_data, REML = FALSE)

# Summarize and Table the Model, Calculate R-squared

summary(multilevel_model_rirs_cli_plot)
tab_model(multilevel_model_rirs_cli_plot)
r_squared <- MuMIn::r.squaredGLMM(multilevel_model_rirs_cli_plot)
print(r_squared)

# Create Simple Slope Plots


## Interaction plot for oth_ori and altruism_potential
plot_oth_ori_alt_pot <- interact_plot(
  multilevel_model_rirs_cli_plot, 
  pred = "altruism_potential", 
  modx = "oth_ori_z_std",
  data = pooled_data, 
  x.label = "Altruism Potential", 
  y.label = "EE Contribution Likelihood",
  modx.labels = c("-1 SD", "Mean", "+1 SD"),  
  colors = c("black", "black", "black"),
  legend.main = "Other-orientation",
  plot.points = TRUE
) + 
  theme(
...
ggsave("plot_oth_ori_alt_pot_black.png", plot = plot_oth_ori_alt_pot, device = "png", width = 20, height = 15, units = "cm", dpi = 300)


## Interaction plot for self_int and benefit_perception
plot_self_int_ben_per <- interact_plot(
  multilevel_model_rirs_cli_plot, 
  pred = "benefit_perception", 
  modx = "self_int_z_std",
  data = pooled_data, 
  x.label = "Benefit Perception", 
  y.label = "EE Contribution Likelihood",
  modx.labels = c("-1 SD", "Mean", "+1 SD"),  
  colors = c("black", "black", "black"),
  legend.main = "Self-interest",
  plot.points = TRUE
) + 
  theme(
    axis.title = element_text(size = 16),  
...
ggsave("plot_self_int_ben_per_black.png", plot = plot_self_int_ben_per, device = "png", width = 20, height = 15, units = "cm", dpi = 300)




# Calculate Simple Slope Analyses
library(reghelper)

simple_slopes(multilevel_model_rirs_cli_plot, pred = c("altruism_potential", "benefit_perception"),
              modx = c("oth_ori_z_std", "self_int_z_std"), confint = TRUE)
