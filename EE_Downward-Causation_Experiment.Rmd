...
ep_founding_scale <- data[,c("ipf_fnd", "ic_fnd")]

ep_developing_scale <- data[,c("ipf_dev", "ic_dev")]

ep_inventing_scale <- data[,c("ipf_inv", "ic_inv")]

ep_scale <- data[,c("ep_fnd", "ep_dev", "ep_inv")]
...
alpha_ep_founding_scale <- alpha(ep_founding_scale)

alpha_ep_developing_scale <- alpha(ep_developing_scale)

alpha_ep_inventing_scale <- alpha(ep_inventing_scale)

alpha_ep_scale <- alpha(ep_scale)
...
alpha_results <- data.frame(
  ...
  Cronbachs_Alpha = c(alpha_general_trust_scale$total$raw_alpha,
                      ...
                      alpha_ep_founding_scale$total$raw_alpha,
                      alpha_ep_developing_scale$total$raw_alpha,
                      alpha_ep_inventing_scale$total$raw_alpha,
                      alpha_ep_scale$total$raw_alpha
                      ))
...
first_round_data <- data |>
  select(1:14, 23:25, 27, 34:37) # Selecting data from first collection round

first_round_data <- first_round_data |>
  pivot_longer(!c(1:6, 15:22), # pivoting the data set to a long format
...
second_round_data <- data |>
  select(1:6, 15:22, 23:25, 27, 34:37) # Selecting data from second collection round
...
data |>
  select(eecontlike, age, gender, pubfund, programpart, startupcenter, gen_trust, self_int, oth_ori, ese, ep) |> #selecting variables
...
pooled_model_2 <- lm(
  eecontlike ~ ee_support_reception + ee_affiliation + ee_relationships + 
    ...
    age + gender + pubfund + programpart + startupcenter + 
    gen_trust * ee_support_reception + 
    gen_trust * ee_relationships +
    oth_ori * altruism_potential +
    ep * positive_outcome_expectation, 
    data = pooled_data )
