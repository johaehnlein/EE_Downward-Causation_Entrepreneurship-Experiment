...
We use the process outlined by Gelman and Stern (2006) to test for a statistically significant difference between the estimate of each independent variable beta in the first round model and that in second round model. This is a simple test to evaluate whether two studies using the same instrument and similar assumptions about the population of interest (e.g., similar random draws) produce statistically similar results. The null hypothesis for the test is that $(\beta_{m1} - \beta_{m2}) = 0$ where $m1$ is the estimate of beta from the first model and $m2$ from the second. 
...
#### Pooled Regression Model (adding controls)

```{r}

# Assuming 'data' is your dataframe
pooled_model_with_controls <- lm(
  eecontlike ~ ee_support_reception + ee_affiliation + ee_relationships + 
    ...
    age + gender + pubfund + programpart + startupcenter, data = pooled_data )

# Display the summary
summary(pooled_model_with_controls)
