...
plot_data$oth_ori_level <- ifelse(plot_data$oth_ori_std <= -1, "Low (-1 SD)", 
                                  ifelse(plot_data$oth_ori_std >= 1, "High (+1 SD)", NA))

plot_data$self_int_level <- ifelse(plot_data$self_int_std <= -1, "Low (-1 SD)", 
                                   ifelse(plot_data$self_int_std >= 1, "High (+1 SD)", NA))

# Filter out the NA values
plot_data <- plot_data %>%
  filter(!is.na(oth_ori_level))



# Plotting the interaction between altruism potential and other orientation
ggplot(plot_data, aes(x = altruism_potential_std, y = eecontlike, linetype = oth_ori_level)) +
  geom_point(alpha = 0.1, color = "black") +
  geom_smooth(data = subset(plot_data, oth_ori_level == "High (+1 SD)"),
              method = "lm", se = FALSE, size = 1.2, color = "black") +
  geom_smooth(data = subset(plot_data, oth_ori_level == "Low (-1 SD)"),
              method = "lm", se = FALSE, size = 1.2, linetype = "dashed", color = "black") +
  scale_linetype_manual(values = c("High (+1 SD)" = "solid", 
                                   "Low (-1 SD)" = "dashed")) +
  labs(title = "Altruism Potential X Other Orientation",
       x = "Altruism Potential",
       y = "EE Contribution Likelihood",
       linetype = "Other Orientation") +
  theme_minimal() + 
  theme(title = element_text(size = 14), axis.title = element_text(size = 12), 
        axis.text = element_text(size = 12), legend.title = element_text(size = 12)) 

# Filter out the NA values
plot_data <- plot_data %>%
  filter(!is.na(self_int_level))

# Plotting the interaction between benefit perception and self interest
ggplot(plot_data, aes(x = benefit_perception_std, y = eecontlike, linetype = self_int_level)) +
  geom_point(alpha = 0.1, color = "black") +
  ...
  labs(title = "Benefit Perception X Self Interest",
       ...
       y = "EE Contribution Likelihood",
       linetype = "Self Interest") +
  theme_minimal() + 
  theme(title = element_text(size = 12), axis.title = element_text(size = 12), 
        axis.text = element_text(size = 12), legend.title = element_text(size = 12))
