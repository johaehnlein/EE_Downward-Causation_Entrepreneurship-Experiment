...
plot_data$oth_ori_level <- cut(plot_data$oth_ori_std, breaks = c(-Inf, -1, 1, Inf), labels = c("Low (-1 SD)", "Mean", "High (+1 SD"))
plot_data$self_int_level <- cut(plot_data$self_int_std, breaks = c(-Inf, -1, 1, Inf), labels = c("Low (-1 SD)", "Mean", "High (+1 SD"))

# Plotting the interaction between altruism potential and other orientation
ggplot(plot_data, aes(x = altruism_potential_std, y = eecontlike, color = oth_ori_level)) +
...
ggplot(plot_data, aes(x = benefit_perception_std, y = eecontlike, color = self_int_level)) +
  geom_point(alpha = 0.1) +
  geom_smooth(method = "lm", se = FALSE, size = 1.2) +
